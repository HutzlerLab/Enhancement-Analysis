...
def process_raw_data_array(folder,file_numbers,abs_ch,fluor_ch):
    ...
    if abs_ch is not None: # check if empty, if not then initialize
        ...
        if not (isinstance(abs_ch,np.ndarray) or isinstance(abs_ch,list)):
            # check if iterable, if not make into list
            abs_ch=[abs_ch]
    if fluor_ch is not None: # check if empty, if not then initialize
        fluor_data = []
        if not (isinstance(fluor_ch,np.ndarray) or isinstance(fluor_ch,list)):
            # check if iterable, if not make into list
            fluor_ch=[fluor_ch]
    for i, num in enumerate(file_numbers):
        processed_data, params = function
