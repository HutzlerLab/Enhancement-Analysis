...
def process_ODs_with_metadata(rawdata_folder,metadata_path,iterate=[],skips=[],ch1_bounds = [1.5,7.5], ch2_bounds=[0,7]):
    ABAB = False
    ABBA = False
    BAAB = False
    if iterate == 'ABAB':
...
def processData(dataset, start_stop1=[0,8],start_stop2=[0,8]):
    Ch1_fig = 1
    plt.figure(Ch1_fig)
    plt.title('Ch1')
    plt.xlabel('Time (ms)')
    ...
    Ch1_int_ODs = integrateODSeries(Ch1_ODs,params,start_stop1,fignum=Ch1_fig)
    ...
    if Skip_Ch2:
        ...
        Ch2_int_ODs = integrateODSeries(Ch2_ODs,params,start_stop2,fignum=Ch2_fig)
        sorted_Ch2_int = sortData(times,Ch2_int_ODs)
        results = [np.array(sorted_Ch1_int), np.array(sorted_Ch2_int), np.array(sorted_times)]
    return results


def process_fluor_OD_with_metadata(rawdata_folder,metadata_path,dtype='ABAB',skips=[],ch1_bounds = [1.5,7.5], ch2_bounds=[0,7]):
    ABAB = False
    ABBA = False
    BAAB = False
    AAAA = False
...
def identifyBUB_max(a1,a2,ch=1,equal=False,returnBool=True):
	...
	if equal:
		...
		if delta > 0:
		    ...
		    _a2 = [a2[i][:-delta] for i in range(len(a2))]
    max1 = [_a1[ch][i].max() for i in range(len(_a1[ch]))]
    max2 = [_a2[ch][i].max() for i in range(len(_a2[ch]))]
