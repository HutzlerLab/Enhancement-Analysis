...
def getRawDataset(folder_path,start_num,stop_num,print_bool,skips=[],initial=False,alternate=False):
    stop_num+=1
...
'''Generate array with time series information'''
def timeArray(parameters):
    dt = parameters[dt_INDEX]
    t0 = parameters[nsample_INDEX]
    npnt = parameters[nsample_INDEX]
    time_ms = np.round(np.linspace(t0,dt*(npnts-1)+t0,npnts),decimals = 6)
    return time_ms

'''Calculate the optical depth (OD) signal from a single channel of a single text file.
	Returns a dataset, which consists of the optical dpeth and acquisition parameters'''
def calculateSingleOD(root_folder,num,ch=1):
    dataset = file2dataset(root_folder,num)

    params = dataset[1]
    OD = raw2OD(np.array(dataset[0][ch-1]),timeArray(params))
    return [OD,params]

def calculateODsSingleChannel(folder,start_num,stop_num,skips=[],ch=1):
    stop_num+=1
    ODs = []
    params = []
    ...
    for i in range(start_num,stop_num):
        if i in skips:
        	...
        	dataset = calculateSingleChannel(folder,i,ch)
        	ODs.append(dataset[0])
        	params.append(dataset[1])
        progress.value = float(i-start_num+1)/(stop_num-start_num)
    dataset = [np.array(ODs),np.array(params)]
    return dataset

def calculateODs(blocked_dataset,unblocked_dataset):
...
def extractTimestampsFixed(params):
	timestamps = []
	for single_params in params:
		if len(timestamps)==0:
			initial = single_params[-1]
			timestamps.append(0)
		else:
			elapsed = single_params[-1] - initial
			timestamps.append(elapsed)
	return np.array(timestamps)


def extractTimestamps(blocked_params,unblocked_params):
    blocked_timestamps = []
    unblocked_timestamps = []
    if blocked_params[0][-2] < unblocked_params[0][-2]:
        blocked_first = True
    else:
        blocked_first = False
    for i in range(len(blocked_params)):
        if blocked_first:
            if len(blocked_timestamps)==0:
                initial = blocked_params[i][-2]
                blocked_timestamps.append(0)
                elapsed_unblocked = unblocked_params[i][-2] - initial
                unblocked_timestamps.append(elapsed_unblocked)
            else:
                elapsed_blocked = blocked_params[i][-2] - initial
                elapsed_unblocked = unblocked_params[i][-2] - initial
                blocked_timestamps.append(elapsed_blocked)
                unblocked_timestamps.append(elapsed_unblocked)
        else:
            if len(unblocked_timestamps)==0:
                initial = unblocked_params[i][-2]
...
def integrateODs(ODs,parameters,start_stop=[0,3],fignum=1):
    num_traces = len(ODs)
...
def processScan(dataset, start_stop_YbOH = [0,3]):
	...
	YbOH_int_ODs = integrateODs(ODs,params,start_stop_YbOH,fignum=fig)
...
def processBUb(blocked_ODs, unblocked_ODs,start_stop_YbOH_blocked=[0,3],start_stop_YbOH_unblocked=[0,3],start_stop_Yb_blocked=[0,3],start_stop_Yb_unblocked=[0,3]):
	...
	for dataset in [blocked_ODs, unblocked_ODs]:
	    ...
	    blocked = params[0][4]
	    if blocked:
	        blocked_YbOH_int = integrateODs(YbOH_OD,params,start_stop_YbOH_blocked,fignum=YbOH_B_fig)
	        blocked_Yb_int = integrateODs(Yb_OD,params,start_stop_Yb_blocked,fignum=Yb_B_fig)
	    elif not blocked:
	        unblocked_YbOH_int = integrateODs(YbOH_OD,params,start_stop_YbOH_unblocked,fignum=YbOH_Ub_fig)
	        unblocked_Yb_int = integrateODs(Yb_OD,params,start_stop_Yb_unblocked,fignum=Yb_Ub_fig)
...
def processPowerData(blocked_ODs, unblocked_ODs,powers,start_stop_YbOH_blocked=[0,3],start_stop_YbOH_unblocked=[0,3],start_stop_Yb_blocked=[0,3],start_stop_Yb_unblocked=[0,3]):
    ...
    sorted_YbOH_blocked = sortData(powers,blocked_int_dataset[0])
    sorted_Yb_blocked = sortData(powers,blocked_int_dataset[1])
    sorted_YbOH_unblocked = sortData(powers,unblocked_int_dataset[0])
    sorted_Yb_unblocked = sortData(poweres,unblocked_int_dataset[1])
    ...
    return [YbOH_enhancement, Yb_enhancement, power_sorted,YbOH_int_data,Yb_int_data]
...
def processTimeDataSingle(dataset,start_stop=[0,3]):
	...
	YbOH_int_ODs = integrateODs(ODs,params,start_stop,fignum=fig)
	times = extractTimestampsFixed(params)
	sorted_int_ODs = sortData(times,YbOH_int_ODs)
	sorted_times = sorted(times)
...
def processAll(folder_path,start_num,stop_num,skips=[],initial_blocked=False,alternate=False,start_stop_YbOH_blocked=[0,2],start_stop_YbOH_unblocked=[0,3],start_stop_Yb_blocked=[0,2],start_stop_Yb_unblocked=[0,3],indep_var=False,print_bool=False):
    ...
    print('Done!')
    print('Integrating...')
    final_data = integrateODs(*ODs,start_stop_YbOH_blocked,start_stop_YbOH_unblocked,start_stop_Yb_blocked,start_stop_Yb_unblocked,indep_var)
    print('Done!')
    return final_data


def getODsfromRaw(folder_path,start_num,stop_num,skips=[],initial_blocked=False,alternate=False,print_bool=False):
    print('Getting raw data...')
    raw= getRawDataset(folder_path,start_num,stop_num,print_bool,skips,initial_blocked,alternate)
    print('Done!')
    print('Calculating ODs...')
    blocked_OD_dataset,unblocked_OD_dataset = calculateODs(*raw)
    print('Done!')
